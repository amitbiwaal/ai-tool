...
export async function POST(
  ...
  try {
    ...
    if (updateError) {
      console.error("Reject review error:", updateError);
      return NextResponse.json({ 
        error: updateError.message || "Failed to reject review",
        details: updateError.details || null
      }, { status: 500 });
    }

    // Recalculate tool rating if review was previously approved
    ...
    return NextResponse.json({ success: true, message: "Review rejected successfully" });
  } catch (error: any) {
    console.error("Unexpected error in reject review route:", error);
    return NextResponse.json({ 
